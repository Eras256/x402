...
On Solana, `upto` uses the [payment-channels program](https://github.com/solana-foundation/payment-channels). The client escrows the maximum amount in an onchain channel at authorization time, and the server settles the actual amount later using a signed voucher. Key roles:

- **Client**: opens the channel and deposits the ceiling amount.
- **Server**: meters usage and signs a settlement voucher for the actual amount via a `receiverAuthorizer` key.
- **Facilitator**: co-signs the `open` transaction as fee/rent payer, and drives settlement by signing `settle_and_seal` as channel `payee`.

The facilitator holds lifecycle authority (can close a channel at its current watermark) but cannot redirect funds or settle a nonzero amount without the server's `receiverAuthorizer` voucher.
