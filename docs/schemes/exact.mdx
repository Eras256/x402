...
<Tabs>
  <Tab title="TypeScript">
    ...
    const resourceServer = new x402ResourceServer(facilitatorClient)
      ...
      .register(
        ...
        // Pass rpcUrl to embed a recent blockhash in the 402 challenge,
        // saving clients an RPC round-trip. Omit to let clients fetch their own.
        new ExactSvmScheme({ rpcUrl: "https://api.devnet.solana.com" }),
      );
  ...
  <Tab title="Go">
    ...
    handler := nethttpmw.X402Payment(nethttpmw.Config{
        ...
        Schemes: []nethttpmw.SchemeConfig{
            ...
            // Pass ServerConfig with RPCURL to embed a recent blockhash in the 402 challenge,
            // saving clients an RPC round-trip. Omit to let clients fetch their own.
            {Network: x402.Network("solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1"), Server: exactsvm.NewExactSvmScheme(&svm.ServerConfig{RPCURL: "https://api.devnet.solana.com"})},
        },
    })(mux)
    ```
  </Tab>
  <Tab title="Python">
    ...
    # Pass rpc_url to embed a recent blockhash in the 402 challenge,
    # saving clients an RPC round-trip. Omit to let clients fetch their own.
    server.register("solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1", ExactSvmServerScheme(rpc_url="https://api.devnet.solana.com"))
