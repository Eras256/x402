...
The `@x402/tvm` package provides the TON mechanism for the `exact` scheme. Clients sign W5R1 `internal_signed` Jetton transfers; the facilitator relays them through a Highload V3 wallet while sponsoring TON gas.
...
const signer = toFacilitatorTvmSigner({
  "tvm:-239": HighloadV3Config.fromPrivateKey(process.env.TVM_PRIVATE_KEY!, {
    provider: "toncenter",
    apiKey: process.env.TONCENTER_API_KEY,
  }),
});
...
facilitator.register("tvm:-239", new ExactTvmScheme(signer));
```

The facilitator wallet must be a Highload V3 wallet and hold at least 1.1 TON before processing payments. Call `scheme.close()` when done with a long-lived client scheme to release cached provider clients.

#### Provider Selection

Toncenter is the default provider. Set `provider: TVM_PROVIDER_TONAPI` to switch to TonAPI:
...
| Provider | Mainnet Base URL | Testnet Base URL |
|----------|-----------------|-----------------|
| Toncenter (default) | `https://toncenter.com` | `https://testnet.toncenter.com` |
| TonAPI | `https://tonapi.io` | `https://testnet.tonapi.io` |
