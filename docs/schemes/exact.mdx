...
### Hedera Facilitator Setup

When running a facilitator for Hedera, you must provide a `FacilitatorHederaSigner` with all required methods wired. Use the built-in helpers from `@x402/hedera`:
...
const hederaKey = PrivateKey.fromStringECDSA(process.env.HEDERA_PRIVATE_KEY!);
...
facilitator.register("hedera:testnet", new ExactHederaScheme(hederaSigner));
```

`verifyPayerSignature` and `preflightTransfer` are both required. Both helpers read from the free Hedera Mirror Node REST API and require no operator-funded queries. You can point them at a custom Mirror Node by passing `{ mirrorNodeUrl: "https://..." }`.

By default, `ExactHederaScheme` rejects payments where `payTo` is an account alias (EVM address or public key alias) rather than an existing `0.0.xxxx` account ID. To allow alias-based destinations:

```typescript
facilitator.register(
  "hedera:testnet",
  new ExactHederaScheme(hederaSigner, { aliasPolicy: "allow" }),
);
```
