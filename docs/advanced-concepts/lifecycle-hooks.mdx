...
* **onPaymentRequired** — Runs when a 402 payment required response is received from a tool call. First hook to return a result wins. Return `{ abort: true }` to cancel, `{ payment }` to supply a pre-built payload, or `void` to proceed with normal payment flow. This hook also runs for corrective 402 responses during payment recovery, so spend controls and abort logic are enforced on every payment attempt.
