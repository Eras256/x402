...
Use the `@x402/tvm` package to instantiate a signer. Requires a W5R1 wallet:
...
const tvmSigner = toClientTvmSigner(keyPair, {
  network: "tvm:-239",  // mainnet; use "tvm:-3" for testnet
...
<Tabs>
  <Tab title="TypeScript">
    ...
    const tvmKeyPair = await mnemonicToPrivateKey(process.env.TON_MNEMONIC!.split(" "));
    const tvmSigner = toClientTvmSigner(tvmKeyPair, {
      network: "tvm:-239",
      apiKey: process.env.TONCENTER_API_KEY,
    });
